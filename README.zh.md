...
0.11.4 版本的本地安装与工具要求 Python 3.9+，且没有 Python 运行时依赖。SSH
目标接受使用 Python 3.8+ 的远程观察载荷。监视 DSH 事件还需要外部 `zstd`
可执行文件。

## 版本 0.11.4

0.11.4 版本提供并发的本地/远程 `watch --all --remote`、持久化的私有发送审计、
...
且串行化的 Release 安装。
0.11.4 新增：`cluster` 无论由哪个来源应答，都会应用自己的时间窗。它此前把时间窗
交给远程一侧，本地一侧却直接拿 daemon 的整份索引来分组，于是 `--all` 形同虚设，
同一条命令还会因为 daemon 是否在应答而给出不同答案——同一台机器上 155 组对 11 组。
DSH 插件也不再接受自己会忽略的 settings 改动：`daemon.*`、`sidecar.*` 与
`stream.*` 是在装配客户端与 supervisor 时一次性读入的，因此插件现在会点名这些键
并指向 profile 的 cordis patch，而不是收下一个什么都不会发生的改动（重载也不会
生效）。自 0.11.3 起，`service install` 不再因为 daemon 仍在建索引而把自己刚装好
的服务拆掉——它像 `daemon start` 一样最多等待首个 ping 45 秒，而不是等满 5 秒就把
任务 bootout、删掉配置，让机器最后连服务都没有。自 0.11.2 起，仍在建立首个索引的
daemon 不再被当成已死的 daemon。它在首扫结束前不应答任何 ping——在 1950 个会话的
机器上这段窗口长达 22 秒——因此 `daemon start` 会等待仍然存活的占用者并接管它，
而不是把输掉归属锁的子进程当成故障；`daemon status`、`daemon stop` 与
`service status` 会指名那个占用者，而不是宣称 daemon 不存在；插件也不再因为索引
太慢而杀掉自己刚拉起的 daemon。
自 0.11.1 起，分析摘要在 6000 字上限内携带最新 24 条时间线条目，而不再是取回
120 条、任由字数上限悄悄决定哪些能留下；窗口之外还有更早的日志时，摘要会如实
说明。自 0.11.0 起，daemon 会针对自己已安装的代码报告漂移——既包括源码树里的
版本号，也包括该树的内容指纹（因为两次发布之间版本号根本不会变），因此
`daemon status` 和 DSH 插件都能说出「长时间运行的 daemon 仍在跑磁盘上已被替换
的代码」。插件也不再夸大自己知道的事：某个时间线来源里不存在的会话不再被当成
来源故障；仅来自内存环形缓冲的历史会如实说明，而不是声称已到时间线起点；分析
超时也会保留模型此前已经生成的文本。
...
- `copilot`：支持 GitHub Copilot CLI 的 `workspace.yaml` 元数据和认证后的
  `--resume --interactive` 发送。0.11.4 版本没有对应事件源，因此状态报告为
  `idle`。
...
省略 `--version v0.11.4` 时会解析最新稳定 GitHub Release。脚本使用 Python 解析
...
如需安装已发布且不可变的修订版，可在对应标签可用后，将标签（例如 `@v0.11.4`）
附加到 Git URL 后。两种方式都会创建隔离环境并安装 `agent-sidecar`；该包没有
Python 运行时依赖。

### 安装 GitHub Release zipapp

如需手动安装，GitHub Releases 会发布可执行 zipapp 及其校验和文件。以 0.11.4
版本为例：
...
0.11.4 版本为受支持数据源提供本地观察、Cursor CLI 事件监视、远程 `list`/`status`
